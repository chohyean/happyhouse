<template>
	<b-container class="bv-example-row mt-3 box">
		<b-row class="mb-1">
			<b-col>
		...
		<b-row class="mb-1">
			<b-col class="text-left">
				<b-button squared variant="info" size="sm" @click="listArticle">목록</b-button>
			</b-col>
			<b-col class="text-right" v-if="article.userid == userInfo.id">
				<b-button squared variant="dark" size="sm" @click="moveModifyArticle" class="mr-2"
					>수정</b-button
				>
				<b-button squared variant="danger" size="sm" @click="deleteArticle">삭제</b-button>
			</b-col>
		</b-row>
			...
			<b-card border-variant="dark" class="mt-2" v-if="!modifyMode">
				<b-row>
					...
					<b-col cols="2" v-if="article.reply == '답변이 없습니다.'"
						><b-button squared class="mr-2" variant="info" size="sm" @click="registerReply"
							>답변하기</b-button
						>
					</b-col>
					<b-col cols="2" v-else>
						<b-button squared class="mr-2" variant="dark" size="sm" @click="registerReply"
							>수정</b-button
						>
						<b-button squared variant="danger" size="sm" @click="deleteReply">삭제</b-button>
					</b-col>
				</b-row>
			</b-card>
			<b-card border-variant="dark" class="mt-2" v-if="modifyMode">
				<b-row>
					<b-col>
						<b-form-input class="text-left" v-model="reply"> {{ article.reply }}</b-form-input>
					</b-col>
					<b-col cols="2"
						><b-button squared class="mr-2" variant="dark" size="sm" @click="modifyReply"
							>수정</b-button
						>
					</b-col>
				</b-row>
...
import { getArticle, updateHit, deleteArticle, modifyReply, deleteReply } from "@/api/board";
